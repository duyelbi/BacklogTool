...
const isEmpty: Validation<string> = (
  str: string,
  onError: Error
): Either<Error, string> => (str !== "" ? Right(str) : Left(onError))

const slice = <A>(items: A[]): A[] => items.slice(0, MAX_LIST_ITEM_COUNT - 1)

const createBacklogClient = (
  space: string,
  domain: string,
  apiKey: string,
  locale: Locale
): Either<Error, BacklogClient> => {
  ...
  return Either.map2(spaceResult, apiKeyResult, (s, a) => {
    return Right(
      new BacklogClientImpl(
        new HttpClient(),
        s,
        domain,
        a,
        new GoogleAppsScriptDateFormatter()
      )
    )
  })
}
...
export const getProject = (
  ...
  const validationResult = isEmpty(
    key,
    Error(Message.PROJECT_KEY_REQUIRED(locale))
  )
  ...
  return Either.map2(validationResult, clientResult, (_, project) =>
    Right(project)
  )
}

export const relatedIssueTypes = (
  issues: List<any>,
...
export const relatedCustomFieldDefinitions = (
  issueTypes: List<IssueType>,
...
export const validate = (
  ...
  const relatedDefinitions = relatedCustomFieldDefinitions(
    relatedTypes,
  ...
  for (let i = 0; i < relatedDefinitions.length; i++) {
    ...
    if (definition.required && !isSupportedCustomField(definition))
      return Left(
        Error(
          Message.VALIDATE_CUSTOM_FIELD_VALUE_IS_REQUIRED_UNSUPPORTED(
            definition.name,
            locale
          )
        )
  ...
  const definitions = customFieldDefinitions.filter((item) =>
    isSupportedCustomField(item)
  )
  ...
  for (let i = 0; i < issues.length; i++) {
    ...
    if (!Option(issue.issueTypeName).isDefined)
      return Left(
        Error(errorString + Message.VALIDATE_ISSUE_TYPE_EMPTY(locale))
      )
    if (issue.parentIssueKey !== undefined && issue.parentIssueKey !== "*")
      if (!client.getIssueV2(issue.parentIssueKey).isDefined)
        return Left(
          Error(
            errorString +
              Message.VALIDATE_PARENT_ISSUE_KEY_NOT_FOUND(
    ...
    for (let j = 0; j < definitions.length; j++) {
      ...
      if (
        definition.required &&
        isEmptyList(definition.applicableIssueTypes) &&
        customField.value === undefined
      )
        return Left(
          Error(
            errorString +
              Message.VALIDATE_CUSTOM_FIELD_VALUE_IS_REQUIRED(
                definition.name,
                locale
              )
          )
      ...
      if (definition.required && customField.value === undefined) {
        for (let k = 0; k < definition.applicableIssueTypes.length; k++) {
          ...
          const issueTypeDefinition = find(
            (issueType: IssueType) => issueType.name === issue.issueTypeName,
            issueTypes
          )
            .orError(
              Error(`Issue type name not found. Name: ${issue.issueTypeName}`)
            )
            .getOrError()
          ...
          if (issueTypeDefinition.id === issueTypeId)
            return Left(
              Error(
                errorString +
                  Message.VALIDATE_CUSTOM_FIELD_VALUE_IS_REQUIRED_ISSUE_TYPE(
                    definition.name,
                    issueTypeDefinition,
                    locale
                  )
        ...
        case 3:
          if (typeof customField.value !== "number")
            return Left(
              Error(
                errorString +
                  Message.VALIDATE_CUSTOM_FIELD_VALUE_IS_NUMBER(
                    definition.name,
                    customField.value,
                    locale
                  )
        ...
        case 4:
          if (!(customField.value instanceof Date))
            return Left(
              Error(
                errorString +
                  Message.VALIDATE_CUSTOM_FIELD_VALUE_IS_DATE(
                    definition.name,
                    customField.value,
                    locale
                  )
...
const getUserProperties = (
  ...
  const lastSpace = spreadSheetService.getUserProperty("space")
    ? spreadSheetService.getUserProperty("space")
    : ""
  const lastDomain = spreadSheetService.getUserProperty("domain")
    ? spreadSheetService.getUserProperty("domain")
    : ".com"
  const lastApiKey = spreadSheetService.getUserProperty("apiKey")
    ? spreadSheetService.getUserProperty("apiKey")
    : ""
  const lastProjectKey = spreadSheetService.getUserProperty("projectKey")
    ? spreadSheetService.getUserProperty("projectKey")
    : ""
...
const createIssue = (
  client: BacklogClient,
  issue: Issue,
  optParentIssueId: Option<string>
): Either<Error, Issue> =>
...
const getTemplateIssuesFromSpreadSheet = (
  spreadSheetService: SpreadSheetService
): Either<Error, any> => {
  let issues = []
  ...
  if (rowLength <= 0)
    return Left(
      Error(Message.INVALID_ROW_LENGTH(spreadSheetService.getUserLocale()))
    )
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    for (let j = 13; j < columnLength; j++) {
      customFields[customFieldIndex] = {
        header: spreadSheetService
          .getRange(sheet, j + 1, ROW_HEADER_INDEX)
          .getFormula(),
...
interface BacklogService {
  ...
  get: (property: UserProperty) => void
...
export const BacklogService = (
  ...
  run: (property: UserProperty): void => {
    ...
    const sheetName =
      getMessage("scriptName", spreadSheetService) + " : " + current
    ...
    const templateIssues =
      getTemplateIssuesFromSpreadSheet(spreadSheetService).getOrError()
    ...
    const convertResults = templateIssues.map((issue) =>
      ...
      optParentIssueId.map(function (parentIssueId) {
        if (parentIssueId === "*") {
          if (previousIssue.flatMap((issue) => issue.parentIssueId).isDefined) {
            previousIssue.map((issue) =>
              showMessage(
                Message.ALREADY_BEEN_CHILD_ISSUE(issue.issueKey, locale),
                spreadSheetService
              )
            )
            optParentIssueId = None<string>()
          } else {
            optParentIssueId = previousIssue.map((issue) =>
              issue.id.toString()
            )
            isTakenOverParentIssueId = true
          }
        } else {
          optParentIssueId = client
            .getIssueV2(parentIssueId)
            .map((issue) => issue.id)
        }
      })
      createIssue(
        ...
        .map((issue) => {
          ...
          if (logSheet == null)
            logSheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet(
              sheetName,
              1
            )
          ...
          const keyCell = spreadSheetService.getRange(
            logSheet,
            LOG_KEY_NUMBER,
            currentRow
          )
          const summaryCell = spreadSheetService.getRange(
            logSheet,
            LOG_SUMMARY_NUMBER,
            currentRow
          )
          ...
          spreadSheetService.setColumnWidth(
            logSheet,
            LOG_SUMMARY_NUMBER,
            summaryWidth
          )
  ...
  init: (property: UserProperty): void => {
    ...
    const locale = spreadSheetService.getUserLocale()
    const templateSheet =
      SpreadsheetApp.getActiveSpreadsheet().getSheetByName(TEMPLATE_SHEET_NAME)
    const lastRowNumber = templateSheet.getLastRow() - 1

    if (lastRowNumber <= 0) throw Error(Message.INVALID_ROW_LENGTH(locale))
    ...
    return createBacklogClient(
      ...
      .map((definition) => {
        const issueTypeRule = SpreadsheetApp.newDataValidation()
          .requireValueInList(definition.issueTypeNames(), true)
          .build()
        const categoryRule = SpreadsheetApp.newDataValidation()
          .requireValueInList(definition.categoryNames(), true)
          .build()
        const versionRule = SpreadsheetApp.newDataValidation()
          .requireValueInList(definition.versionNames(), true)
          .build()
        const priorityRule = SpreadsheetApp.newDataValidation()
          .requireValueInList(definition.priorityNames(), true)
          .build()
        const userRule = SpreadsheetApp.newDataValidation()
          .requireValueInList(definition.userNames(), true)
          .build()
        const customFieldStartColumnNumber = 14 // N ~
        let currentColumnNumber = customFieldStartColumnNumber

        templateSheet
          .getRange(2, 7, lastRowNumber)
          .setDataValidation(issueTypeRule) // 7 = G
        templateSheet
          .getRange(2, 8, lastRowNumber)
          .setDataValidation(categoryRule) // 8 = H
        templateSheet
          .getRange(2, 9, lastRowNumber)
          .setDataValidation(versionRule) // 9 = I
        templateSheet
          .getRange(2, 10, lastRowNumber)
          .setDataValidation(versionRule) // 10 = J
        templateSheet
          .getRange(2, 11, lastRowNumber)
          .setDataValidation(priorityRule) // 11 = K
        templateSheet
          .getRange(2, 12, lastRowNumber)
          .setDataValidation(userRule) // 12 = L
        for (let i = 0; i < definition.customFields.length; i++) {
          const customField = definition.customFields[i]
          const headerCell = spreadSheetService.getRange(
            templateSheet,
            currentColumnNumber,
            ROW_HEADER_INDEX
          )
          ...
          if (!isSupportedCustomField(customField)) continue
          ...
          headerCell.setFormula(
            "=hyperlink(\"" +
              ...
              ".backlog" +
              property.domain +
              "/EditAttribute.action?attribute.id=" +
              customField.id +
              "\";\"" +
              headerName +
              "\")"
            ...
            definition.customFieldItemNames(customField).map((itemNames) => {
              if (itemNames.length > 0) {
                ...
                templateSheet
                  .getRange(
                    2,
                    validationRuleIndex + customFieldStartColumnNumber,
                    lastRowNumber
                  )
  ...
  get: (property: UserProperty): void => {
    ...
    const issues = client.getIssuesV2(project.id)
    Logger.log("Issues", issues)
    let sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheetName)
    if (sheet == null) {
      sheet = SpreadsheetApp.getActiveSpreadsheet().insertSheet(sheetName, 1)
    } else {
    ...
    showMessage(
      getMessage("complete_get_issues", spreadSheetService),
      spreadSheetService
    )
  },
